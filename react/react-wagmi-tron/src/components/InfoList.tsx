...
interface InfoListProps {
  evmSignedMsg: string;
  tronSignedMsg: string;
}

export const InfoList = ({ evmSignedMsg, tronSignedMsg }: InfoListProps) => {
  const kitTheme = useAppKitTheme();
  const state = useAppKitState();
  const { address, caipAddress, isConnected, status, embeddedWalletInfo } = useAppKitAccount();
  const eip155AccountState = useAppKitAccount({ namespace: 'eip155' }) // for EVM
  const tronAccountState = useAppKitAccount({ namespace: 'tron' }) // for Tron
    ...
    <>
      <section>
        ...
        <pre>
          ...
          Address Tron: {tronAccountState.address}<br />
        </pre>
      </section>

      {(evmSignedMsg || tronSignedMsg) && (
        <section>
          <h2>Sign msg</h2>
          <pre>
            EVM signature: {evmSignedMsg}<br />
            Tron signature: {tronSignedMsg}<br />
          </pre>
        </section>
      )}

      <section>
        <h2>useAppKitAccount</h2>
        <pre>
          Address: {address}<br />
